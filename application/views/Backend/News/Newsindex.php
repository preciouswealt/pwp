                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <!-- <td>
                                </?php if (strlen($new->detail) > 70) { ?>
                                <p><b></?php echo  mb_substr($new->detail, 0, 70,'UTF-8') . "..."; ?></b></p>
                                </?php  } else { ?>
                                <p><b></?php echo  $new->detail ?></b></p>
                                </?php  } ?>
                            </td> -->
                            <td><?php echo $new->savedate ?></td>
                            <td><?php echo $new->update_date ?></td>
                            <td>
                                <!-- </?php echo $new->status_new ?> -->
                                <div id="Status<?php echo  $new->id;?>">
                                    <?php if($new->status_new== 0) {?>
                                    <button type="button" class="btn btn-danger btn-xs"
                                        style="color:white;font-size: 13px;border-radius: 5px;"
                                        onclick="statusupdate(<?php echo $new->id ;?>)">
                                        ปิดการใช้งาน
                                    </button>
                                    <?php }else if($new->status_new == 1) { ?>
                            ...
                            <td><?php echo $new->image_new ?></td>
                            <td><button type="button" onclick="showdetail(id = '<?php echo $new->id ?>')"
                                    class="btn btn-primary" data-toggle="modal"
                                    data-target="#exampleModal">ดูรายละเอียด</button></td>

                            <td>
                                <button type="button" class="btn btn-danger btn-xs"
                                    style="color:white;font-size: 13px;border-radius: 5px;"
                                    onclick="deletenews(id ='<?php echo $new->id ;?>', imag = '<?php echo $new->image_new ?>')">
                                    ลบ
                                </button>
                            </td>
                        </tr>
